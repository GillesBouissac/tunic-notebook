// Code from https://github.com/DougAnderson444/panzoom-node
import pinchZoom from './pinchZoom';
import type { Action } from 'svelte/action';

// Svelte action directive
// see: https://svelte.dev/docs#template-syntax-element-directives-use-action
export const pzoom: Action<HTMLDivElement, { handle?: HTMLElement, panAnywhere: boolean } | undefined> = (node, params) => {
	let container = node.parentElement || document.body;

	// ensure touch and select action defaults are disabled
	container.style.touchAction = 'none'; // disable default mobile pin zoom
	container.style.userSelect = 'none'; // no highlighting
	...
	node.style.touchAction = 'none'; // disable default mobile pin zoom
